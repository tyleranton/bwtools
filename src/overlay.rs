...
pub fn write_if_changed(path: &Path, last_text: &mut Option<String>, text: String) {
    if last_text.as_deref() == Some(text.as_str()) { return; }
...
pub fn write_rating(cfg: &Config, app: &mut App) {
    ...
    write_if_changed(&cfg.rating_output_path, &mut app.rating_output_last_text, text);
}
...
pub fn write_opponent(cfg: &Config, app: &mut App) {
    ...
    let race = app.opponent_race.clone().unwrap_or_else(|| "Unknown".to_string());
    let rating_opt = app
        .opponent_toons_data
        .iter()
    ...
    write_if_changed(&cfg.opponent_output_path, &mut app.opponent_output_last_text, text);
}
